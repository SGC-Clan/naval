...
partial class NavalPlayer : Player
{
	...
	{
		if ( ConsoleSystem.Caller.Pawn is NavalPlayer basePlayer )
		{
			if ( basePlayer.DevController is NavalNoclipController )
			{
				basePlayer.DevController = null;
			}
			else
			{
				basePlayer.DevController = new NavalNoclipController();
			}
		}
	}
	...
	void SimulateAnimation( PawnController controller )
	{
		...
		animHelper.IsSwimming = this.GetNavalWaterLevel() >= 0.5f;
